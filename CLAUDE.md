...
- The **`bundle-tools`** cargo feature (implies `static-ffmpeg`) embeds `yt-dlp` and the `ffmpeg` CLI binary directly into the exe at compile time and extracts them to a per-user managed dir on first run. Used by the Windows CI job to produce a single `.exe` with no companion files. Requires `BUNDLE_YTDLP=<path>` and `BUNDLE_FFMPEG_CLI=<path>` to be set at build time. Build: `BUNDLE_YTDLP=... BUNDLE_FFMPEG_CLI=... cargo build --release --features bundle-tools`.
...
The CI (`windows-latest` runner) produces a single self-contained `yt-dlp-clipper-<ver>-win64.exe`; whole binary is GPL. The `bundle-tools` feature bakes `yt-dlp.exe` and a static `ffmpeg.exe` CLI (gyan `full_build`, no DLL deps) into the binary at compile time; `static-ffmpeg` (implied by `bundle-tools`) links libav* statically so there are no DLL deps from the app either. The result is a truly standalone exe. First run extracts the embedded tool binaries to the per-user managed dir (`storage_dir/bin/`), where they persist and can be updated by `yt-dlp -U`.
- **CI build**: MSYS2 UCRT64 provides `make`/`nasm` (for FFmpeg's build system) and static codec libs (`x264`, `libvpx`, `opus`, `lame`) via pacman. Rust target is `x86_64-pc-windows-gnu`; `-C target-feature=+crt-static` links libgcc/winpthread statically, so the exe depends only on Windows system DLLs (always present). `gyan 6.0 full_build.7z` supplies the embedded `ffmpeg.exe`.
