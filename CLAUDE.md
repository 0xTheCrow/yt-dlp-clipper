...
#### Windows single exe (built natively in CI; cross-compile script for devs)
The CI (`windows-latest` runner) produces a single self-contained `yt-dlp-clipper-<ver>-win64.exe`; whole binary is GPL. The `bundle-tools` feature bakes `yt-dlp.exe` and a static `ffmpeg.exe` CLI (gyan `full_build`, no DLL deps) into the binary at compile time; static libav* links (via `FFMPEG_DIR` pointing at a pre-built install) mean no DLL deps from the app either. The result is a truly standalone exe. First run extracts the embedded tool binaries to the per-user managed dir (`storage_dir/bin/`), where they persist and can be updated by `yt-dlp -U`.
- **CI build**: MSYS2 UCRT64 provides `make`/`nasm`/`clang` and static codec libs (`x264`, `libvpx`, `opus`, `lame`). The CI job first builds FFmpeg 6.0 statically in an MSYS2 shell (cached between runs), then sets `FFMPEG_DIR` (Windows path to the install) and `CODEC_LIB_DIR` (Windows path to MSYS2's lib dir, for the codec .a files) before running cargo. `ffmpeg-sys-the-third`'s `FFMPEG_DIR` path skips `./configure` entirely and just emits link-search + include paths. Our `build.rs` emits the codec transitive link deps (`x264`, `vpx`, `opus`, `mp3lame`) and Windows system libs (`bcrypt`, `ws2_32`, etc.) that the FFMPEG_DIR path doesn't add. Rust target is `x86_64-pc-windows-gnu`; `-C target-feature=+crt-static` links libgcc/winpthread statically, so the exe depends only on Windows system DLLs (always present). `gyan 6.0 full_build.7z` supplies the embedded `ffmpeg.exe`.
