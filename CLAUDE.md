...
#### Windows single exe (cross-compiled from Linux in CI; cross-compile script for devs)
The CI (`ubuntu-22.04` runner) cross-compiles a single self-contained `yt-dlp-clipper-<ver>-win64.exe`; whole binary is GPL. The `bundle-tools` feature bakes `yt-dlp.exe` and a static `ffmpeg.exe` CLI (gyan `full_build`, no DLL deps) into the binary at compile time; static libav* links (via `FFMPEG_DIR` pointing at a pre-built install) mean no DLL deps from the app either. The result is a truly standalone exe. First run extracts the embedded tool binaries to the per-user managed dir (`storage_dir/bin/`), where they persist and can be updated by `yt-dlp -U`.
- **CI build**: `gcc-mingw-w64-x86-64` (cross-compiler) + `nasm` on Ubuntu. The CI job first cross-compiles the codec libs (x264, libvpx, opus, lame) and FFmpeg 6.0 statically for Windows (all cached between runs), then sets `FFMPEG_DIR` and `CODEC_LIB_DIR` before running cargo. `ffmpeg-sys-the-third`'s `FFMPEG_DIR` path skips `./configure` entirely and just emits link-search + include paths. Our `build.rs` emits the codec transitive link deps (`x264`, `vpx`, `opus`, `mp3lame`) and Windows system libs (`bcrypt`, `ws2_32`, etc.) that the FFMPEG_DIR path doesn't add. Rust target is `x86_64-pc-windows-gnu`; `-C target-feature=+crt-static` links libgcc/winpthread statically, so the exe depends only on Windows system DLLs (always present). `gyan 6.0 full_build.7z` supplies the embedded `ffmpeg.exe`. `BINDGEN_EXTRA_CLANG_ARGS` points clang at the MinGW Windows headers (`/usr/x86_64-w64-mingw32/include`) so `ffmpeg-sys-the-third`'s bindgen pass generates correct FFI bindings for the Windows target.
- **Why Linux not Windows runner**: FFmpeg's `./configure` is a bash script that detects the build host via `uname`. MSYS2 on Windows presents a POSIX-like shell, so configure treats MinGW GCC as a cross-compiler and fails the compiler test even with `--enable-cross-compile`. Cross-compiling from Linux with `x86_64-w64-mingw32-gcc` is the standard, reliable path for building FFmpeg for Windows.
